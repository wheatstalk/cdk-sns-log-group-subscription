 ...
 */
export interface LogGroupSubscriptionProps extends aws_sns_subscriptions.SubscriptionProps {
  /**
...
export class LogGroupSubscription implements aws_sns.ITopicSubscription {
  ...
  bind(topic: aws_sns.ITopic): aws_sns.TopicSubscriptionConfig {
    ...
    const lambda = new LogFunction(topic, `LogGroupSubscription${logGroupUniqueness}`, {
      logGroup,
      logFormat: this.logFormat,
    });

    return new aws_sns_subscriptions.LambdaSubscription(lambda, this.subscriptionProps).bind(topic);
  }
}
...
class LogFunction extends aws_lambda.Function {
  constructor(scope: Construct, id: string, props: LogFunctionProps) {
    ...
    super(scope, id, {
      ...
      code: aws_lambda.Code.fromAsset(path.join(__dirname, '../assets/log.lambda')),
      environment: {
